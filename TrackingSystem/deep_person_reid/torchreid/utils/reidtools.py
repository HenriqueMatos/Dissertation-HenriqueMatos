...
def visualize_ranked_results(
    ...
    for q_idx in range(num_q):
        ...
        for g_idx in indices[q_idx, :]:
            ...
            if not invalid:
                ...
                # print(gallery[g_idx][:3][0].split('/')[3])
                # print((1-distmat[q_idx, g_idx])*100)
                if gallery[g_idx][:3][0].split('/')[-2] in IDsObjectList.keys():
                    IDsObjectList[gallery[g_idx][:3][0].split('/')[-2]].append((1-distmat[q_idx, g_idx])*100)
                else:
                    IDsObjectList[gallery[g_idx][:3][0].split('/')[-2]]=[(1-distmat[q_idx, g_idx])*100]
                    
                # IdsList.append((1-distmat[q_idx, g_idx])*100)
                # if ((1-distmat[q_idx,g_idx])*100)>20:
                # if IDsProbability.__contains__(gpid):
                #     IDsProbability[gpid]+=1
                # else:
                #     IDsProbability[gpid]=1
    ...
    # # print(IDsProbability)
    # print("Media ", statistics.fmean(IdsList))
    # print("Mediana ", statistics.median(IdsList))
    # print("Moda ", statistics.mode(IdsList))
    # print("Variancia ", statistics.pvariance(IdsList))
    # print("Desvio ", statistics.pstdev(IdsList))

    # f = open("demofile2.txt", "a")
    # f.write("ID: "+gallery[g_idx][:3][0].split('/')[-2]+"\n")
    # f.write("Media "+ str(statistics.fmean(IdsList))+"\n")
    # f.write("Mediana "+ str(statistics.median(IdsList))+"\n")
    # f.write("Moda "+ str(statistics.mode(IdsList))+"\n")
    # f.write("Variancia "+ str(statistics.pvariance(IdsList))+"\n")
    # f.write("Desvio "+ str(statistics.pstdev(IdsList))+"\n")

    # f.close()

    print('Done. Images have been saved to "{}" ...'.format(save_dir))
    
    return IDsObjectList
